...
// ---------- recent dictations (last 10) ----------
// Merges two local sources: dictations made in this web app (localStorage)
// and dictations made through the browser extension, which the extension's
// content script serves over postMessage on localhost pages.
...
function extensionHistory(timeoutMs = 500) {
  return new Promise((resolve) => {
    const timer = setTimeout(() => {
      window.removeEventListener("message", onMsg);
      resolve([]); // extension not installed / no content script — fine
    }, timeoutMs);
    function onMsg(e) {
      if (e.source !== window || e.data?.type !== "wisprgemma-history") return;
      clearTimeout(timer);
      window.removeEventListener("message", onMsg);
      resolve(Array.isArray(e.data.entries) ? e.data.entries : []);
    }
    window.addEventListener("message", onMsg);
    window.postMessage({ type: "wisprgemma-get-history", limit: RECENT_LIMIT }, window.location.origin);
  });
}

async function renderRecent() {
  const seen = new Set();
  const all = [...(await extensionHistory()), ...localHistory()]
    .sort((a, b) => b.ts - a.ts)
    .filter((h) => {
      const k = `${h.ts}:${h.text}`;
  ...
  $("recent-section").hidden = all.length === 0;
  $("recent").innerHTML = all
    .map((h) => {
      ...
      return `<li>
        <div class="recent-meta"><time>${when}</time>${
          MODE_NAMES[h.mode] ? `<span class="recent-mode">${MODE_NAMES[h.mode]}</span>` : ""
        }</div>
        <p>${esc(h.text)}</p>
      </li>`;
    })
